...
describe('runAgyCliStep', () => {
  ...
  it('carries the queue wait out on `_cli` and logs the admission', async () => {
    ...
    // held. The log line is for the worker; `queuedMs` is for the archive.
    process.env.AGY_CLI_MAX_CONCURRENT = '1';
    process.env.AGY_CLI_BIN = writeFakeAgy(
      `setTimeout(() => { out(${JSON.stringify(SUCCESS_ENVELOPE)}); }, 2500);`,
    );
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
    try {
      const first = runStep({ timeoutMs: 60_000 });
      // Let the first step take the only slot before the second asks for one.
      await new Promise((r) => setTimeout(r, 250));
      const second = runStep({ timeoutMs: 60_000 });
      const [ran, queued] = await Promise.all([first, second]);

      type CliBag = Record<string, { queuedMs: number }>;
      expect((ran.result['data._cli'] as CliBag)['data.out'].queuedMs).toBeLessThan(100);
      expect((queued.result['data._cli'] as CliBag)['data.out'].queuedMs).toBeGreaterThan(1000);
