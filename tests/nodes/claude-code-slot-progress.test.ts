/**
 * A QUEUED claude-code step is not a STALLED run.
 *
 * The executor serialises CLI children per replica (`CLAUDE_CODE_MAX_CONCURRENT`,
 * default 1) and the run-level watchdog interrupts a run whose `lastProgressAt`
 * has not moved for 30 minutes. A step waiting for a slot writes nothing until
 * its CLI starts, so a run that waited longer than the stale window was killed
 * before it had spawned anything at all (run_1789454327425_zvof94, 2026-09-15).
 *
 * The fix is one tick a minute, on the channel the step already has, for as
 * long as it is queued and not one tick longer. This drives the real semaphore
 * with fake timers — no CLI, no Redis, no publisher.
 */
...
const OLD_MAX = process.env.CLAUDE_CODE_MAX_CONCURRENT;
...
afterEach(() => {
  ...
  while (__claudeCodeSlotsInUse() > 0) releaseSlot();
  if (OLD_MAX === undefined) delete process.env.CLAUDE_CODE_MAX_CONCURRENT;
  else process.env.CLAUDE_CODE_MAX_CONCURRENT = OLD_MAX;
});

describe('claude-code slot queue — progress while waiting', () => {
  it('reports once per interval while queued, and stops the moment it is admitted', async () => {
    ...
    expect(__claudeCodeSlotsInUse()).toBe(1);
  ...
  it('never reports for a step that gets a slot straight away, and stops on a queue timeout', async () => {
    ...
    const assertion = expect(doomed).rejects.toMatchObject({ code: 'claude_code_queue_timeout' });
