...
import { toolOk, toolError, resolveLifecycleQueue, resolveRunUserId } from './workspace-common';
...
const tool: NativeToolDefinition = {
  ...
  handler: async (args: AnyObject, context: NativeToolContext): Promise<NativeMcpResult> => {
    ...
    if (!/^https:\/\/github\.com\/[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+\/pull\/\d+/.test(prUrl)) {
    ...
    try {
      const result = await resolveLifecycleQueue(context).runJob(
        WORKSPACE_QUEUE,
        'merge',
        { action: 'merge', prUrl, ownerUserId, timeoutMs, mergeMethod },
        timeoutMs + 60 * 1000
      );
