...
function delegatedFromUserId(state: any): string | null {
  ...
  const owner = state?.userId || state?.data?.userId;
  return typeof owner === 'string' && owner && owner !== caller ? owner : null;
}
...
export async function acquireWorkspaceForStep(state: any): Promise<WorkspaceSession | null> {
  ...
  const delegatedFrom = delegatedFromUserId(state);
  const session = await acquireWorkspace(db, {
    ...
    // Audit only, and only on a delegated run: the workspace itself already
    // belongs to the caller, so every credential this spawn uses is the
    // caller's. This just names the automation owner in the worker's log.
    ...(delegatedFrom ? { delegatedFromUserId: delegatedFrom } : {}),
  });
          ...
          if (parserDef) {
            ...
            const parserToolExecutor: ParserToolExecutor = async (toolName, params) => {
              if (nativeReg.has(toolName)) {
                // SECURITY: dispatch through `callTool`, never `tool.handler`.
                // `callTool` is the single chokepoint where the run's
                // capability profile (`enforceToolCapability`) and the exec
                // runtime gates (`runExecGuard`: kill switch, EXEC_RATE_MAX,
                // fail-closed audit) are applied — calling the handler directly
                // skipped both for every parser-dispatched tool.
                //
                // Parser-driven calls are built from the model's own streamed
                // output, so the args are model-controlled: `untrustedCaller`
                // is unconditional, which keeps `fetch_url` from attaching the
                // run's internal credentials. See
                // NativeToolContext.untrustedCaller.
                return nativeReg.callTool(toolName, params, {
                  publisher: null,
                  state,
                  runId: callRunId ?? null,
                  nodeId: null,
                  toolId: null,
            ...
            neuronParser.setContext({
              ...
              // Run-resolved secrets, so parser outputs and parser tool steps
              // can reference a credential by name instead of carrying its
              // value in the node document.
              _secrets: inputData._secrets || null,
            });
