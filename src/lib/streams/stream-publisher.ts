...
export class StreamEventPublisher {
  ...
  /** User speech transcription arriving from the realtime provider / STT engine. */
  async inputTranscription(
    text: string,
    ...
    await this.publish({
      ...
      sessionId: this.sessionId,
      text,
      ...(options?.suppressed ? { suppressed: true } : {}),
      timestamp: Date.now(),
    });
  }

  /** Assistant speech transcription generated by the realtime engine. */
