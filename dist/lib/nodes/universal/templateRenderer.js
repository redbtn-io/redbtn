...
exports.hasGlobalStateVariables = hasGlobalStateVariables;
exports.extractTemplateVariables = extractTemplateVariables;
exports.renderTemplateAsync = renderTemplateAsync;
exports.renderParametersAsync = renderParametersAsync;
exports.prefetchGlobalStateForTemplate = prefetchGlobalStateForTemplate;
 ...
 */
function renderParameters(parameters, state) {
    const rendered = {};
 ...
 */
function hasGlobalStateVariables(str) {
...
function extractTemplateVariables(template) {
    ...
    return Array.from(matches, match => ({ type: match[1], path: match[2] }));
}
/**
 ...
 */
function renderTemplateAsync(template, state) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const replacements = yield Promise.all(globalStateMatches.map((match) => __awaiter(this, void 0, void 0, function* () {
            const fullMatch = match[0];
            const path = match[1];
            const value = yield client.resolveTemplatePath(path);
 ...
 */
function renderParametersAsync(parameters, state) {
    return __awaiter(this, void 0, void 0, function* () {
        const rendered = {};
        for (const [key, value] of Object.entries(parameters)) {
            if (typeof value === 'string' && hasTemplateVariables(value)) {
                ...
                if (hasGlobalStateVariables(value)) {
                    rendered[key] = yield renderTemplateAsync(value, state);
                }
                else {
                ...
                // Recursively render nested objects
                rendered[key] = yield renderParametersAsync(value, state);
            }
            else {
 ...
 */
function prefetchGlobalStateForTemplate(template) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (namespaces.size === 0)
            return;
        const client = getGlobalStateClient();
        yield Promise.all(Array.from(namespaces).map(ns => client.prefetch(ns)));
    });
}
