...
exports.executeNeuron = executeNeuron;
const templateRenderer_1 = require("../templateRenderer");
const errorHandler_1 = require("./errorHandler");
const audio_stream_1 = require("../../../tts/audio-stream");
...
function normalizeMessages(messages) {
    ...
    for (const msg of messages) {
        if (msg.role === 'system') {
            ...
            nonSystemMessages.push(Object.assign({}, msg));
        }
    }
    ...
    for (const msg of nonSystemMessages) {
        ...
        if (lastMsg && lastMsg.role === msg.role) {
            ...
            normalized.push(Object.assign({}, msg));
        }
    }
    return normalized;
}
 ...
 */
function resolveConfigValue(value, state) {
    ...
    if (paramMatch && state.parameters) {
        ...
        if (resolved !== undefined) {
            if (DEBUG)
                console.log(`[NeuronExecutor] Resolved parameter ${paramName}:`, resolved);
            return resolved;
        }
    }
    ...
    if (stateMatch) {
        ...
        const resolved = (0, templateRenderer_1.getNestedProperty)(state, path);
        if (resolved !== undefined) {
            if (DEBUG)
                console.log(`[NeuronExecutor] Resolved state path ${path}:`, resolved);
            return resolved;
        }
    }
 ...
 */
function executeNeuron(config, state) {
    return __awaiter(this, void 0, void 0, function* () {
        // If error handling configured, wrap execution
        if (config.errorHandling) {
            const result = yield (0, errorHandler_1.executeWithErrorHandling)(() => executeNeuronInternal(config, state), config.errorHandling, {
                type: 'neuron',
 ...
 * @returns Partial state with output field set to LLM response
 */
function executeNeuronInternal(config, state) {
    return __awaiter(this, void 0, void 0, function* () {
        var _a, _b, _c, _d, _e;
        ...
        console.log('[NeuronExecutor] config:', {
            ...
            userPromptPreview: (_a = config.userPrompt) === null || _a === void 0 ? void 0 : _a.substring(0, 100),
        ...
        try {
            ...
            const neuronId = config.neuronId || state.defaultNeuronId || ((_b = state.data) === null || _b === void 0 ? void 0 : _b.defaultNeuronId);
            if (!neuronId) {
            ...
            const userId = state.userId || ((_c = state.data) === null || _c === void 0 ? void 0 : _c.userId);
            let model = yield neuronRegistry.getModel(neuronId, userId, Object.keys(modelOverrides).length > 0 ? modelOverrides : undefined);
            if (!model) {
            ...
            if (config.structuredOutput) {
                ...
                if (isOllamaModel) {
                    ...
                    if (DEBUG)
                        console.log('[NeuronExecutor] Will use Ollama native format at invocation');
                }
                else {
            ...
            if (messagesFieldMatch) {
                ...
                const messagesArray = (0, templateRenderer_1.getNestedProperty)(state, fieldName);
                ...
                if (Array.isArray(messagesArray)) {
                    ...
                    if (config.systemPrompt || state.systemPrefix) {
                        let systemPrompt = config.systemPrompt
                            ? (0, templateRenderer_1.renderTemplate)(config.systemPrompt, state)
                            : '';
                ...
                const userPrompt = (0, templateRenderer_1.renderTemplate)(config.userPrompt, state);
                if (DEBUG)
            ...
            if (config.structuredOutput) {
                ...
                if (useNativeFormat) {
                    ...
                    rawResponse = yield model.invoke(messages, {
                ...
                console.log('[NeuronExecutor] RAW messages BEFORE normalization:', {
                    ...
                    previews: messages.map((m, i) => {
                        var _a, _b;
                        return ({
                            idx: i,
                            role: m.role,
                            contentStart: (_a = m.content) === null || _a === void 0 ? void 0 : _a.substring(0, 80),
                            contentLength: (_b = m.content) === null || _b === void 0 ? void 0 : _b.length
                        });
                    })
                });
                ...
                const totalChars = normalizedMessages.reduce((sum, m) => { var _a; return sum + (((_a = m.content) === null || _a === void 0 ? void 0 : _a.length) || 0); }, 0);
                ...
                const stream = yield Promise.race([streamPromise, timeoutPromise]);
                ...
                if (streamToUser && state.runPublisher) {
                    try {
                        const neuronConfig = yield neuronRegistry.getConfig(neuronId, userId);
                        if (neuronConfig.audioOptimized) {
                            audioPipeline = new audio_stream_1.AudioStreamPipeline({
                                publisher: state.runPublisher,
                                ttsOptions: {
                                    voice: ((_d = state.data) === null || _d === void 0 ? void 0 : _d.ttsVoice) || undefined,
                                    speed: ((_e = state.data) === null || _e === void 0 ? void 0 : _e.ttsSpeed) || undefined,
                                },
                            });
                ...
                const resolvedInactivityTimeout = resolveConfigValue(config.inactivityTimeout, state);
                const STREAM_INACTIVITY_TIMEOUT = typeof resolvedInactivityTimeout === 'number' && resolvedInactivityTimeout > 0
                ...
                const makeInactivityPromise = () => new Promise((_, reject) => {
                    inactivityTimerId = setTimeout(() => {
                        console.error(`[NeuronExecutor] Stream stalled — no token received for ${STREAM_INACTIVITY_TIMEOUT / 1000}s`, {
                            neuronId: config.neuronId,
                            outputField: config.outputField,
                            chunksReceivedBeforeStall: chunkCount,
                            elapsedMs: Date.now() - streamStartTime
                        });
                        reject(new Error(`LLM stream stalled — no output for ${STREAM_INACTIVITY_TIMEOUT / 1000} seconds`));
                ...
                try {
                    ...
                    while (true) {
                        ...
                        let iterResult;
                        try {
                            iterResult = yield Promise.race([asyncIter.next(), inactivityPromise]);
                        }
                        finally {
