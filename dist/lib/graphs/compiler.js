...
exports.GraphCompilationError = void 0;
exports.compileGraphFromConfig = compileGraphFromConfig;
 ...
 */
function createConfigurableNode(graphNodeId, config = {}) {
    return (state) => __awaiter(this, void 0, void 0, function* () {
        ...
        const enhancedState = Object.assign(Object.assign({}, state), { nodeConfig: Object.assign(Object.assign({}, config), { graphNodeId, nodeId: config.nodeId || graphNodeId }) });
        return yield universalNode(enhancedState);
    });
}
/**
 ...
 */
function compileGraphFromConfig(config) {
    var _a;
    ...
    const builder = new langgraph_1.StateGraph(RedGraphState);
    ...
    const joinTargets = new Set((joinEdge === null || joinEdge === void 0 ? void 0 : joinEdge.join) || []);
    ...
    for (const node of config.nodes) {
        ...
        console.log(`[GraphCompiler]   Adding node: ${node.id} (nodeId: ${((_a = node.config) === null || _a === void 0 ? void 0 : _a.nodeId) || node.id})`);
...
function validateGraphConfig(config) {
    ...
    for (const node of config.nodes || []) {
        if (!((_a = node.config) === null || _a === void 0 ? void 0 : _a.nodeId) && !node.id) {
    ...
    // Throw if any errors
    if (errors.length > 0) {
        throw new GraphCompilationError(`Graph validation failed:\n${errors.map(e => `  - ${e}`).join('\n')}`, config.graphId);
    }
}
/**
 ...
 */
function traceChain(startNodeId, edges, joinTargets) {
    const chain = [startNodeId];
    let current = startNodeId;
    while (true) {
        // Find a simple edge from current (not parallel, not conditional, not join)
        const nextEdge = edges.find(e => e.from === current && e.to && !e.parallel && !e.condition && !e.targets && !e.join);
        if (!nextEdge || !nextEdge.to)
            break;
        const next = nextEdge.to;
        // Stop if we've reached __end__
        if (next === '__end__' || next === langgraph_1.END)
            break;
        chain.push(next);
        // Stop after adding a join target — it's the last node in this chain
        if (joinTargets.has(next))
            break;
        current = next;
    }
    return chain;
 ...
 */
function addFanOutEdge(builder, edge, config) {
    ...
    for (const target of targets) {
        ...
        if (chain.length === 1) {
            ...
            const subBuilder = new langgraph_1.StateGraph(RedGraphState);
            for (const nodeId of chain) {
                const nodeConfig = config.nodes.find(n => n.id === nodeId);
                if (nodeConfig) {
                    const wrappedFn = createConfigurableNode(nodeId, nodeConfig.config || {});
                    subBuilder.addNode(nodeId, wrappedFn);
                }
            }
            // Add sequential edges within the subgraph
            subBuilder.addEdge('__start__', chain[0]);
            for (let i = 0; i < chain.length - 1; i++) {
                subBuilder.addEdge(chain[i], chain[i + 1]);
            }
            subBuilder.addEdge(chain[chain.length - 1], '__end__');
            ...
            if (joinEdge === null || joinEdge === void 0 ? void 0 : joinEdge.join) {
    ...
    builder.addConditionalEdges(edge.from, (state) => {
        var _a;
        if (((_a = state.data) === null || _a === void 0 ? void 0 : _a.nextGraph) === 'error_handler') {
 ...
 */
function addJoinEdge(builder, edge) {
 ...
 */
function addSimpleEdge(builder, edge) {
    const to = edge.to || langgraph_1.END;
 ...
 */
function addConditionalEdge(builder, edge, config) {
    if (!edge.condition && !edge.targets) {
        throw new GraphCompilationError(`Conditional edge from ${edge.from} missing condition or targets`, config.graphId);
    }
    ...
    targetMap['__end__'] = langgraph_1.END;
 ...
 */
class GraphCompilationError extends Error {
    constructor(message, graphId) {
        super(message);
...
exports.GraphCompilationError = GraphCompilationError;
